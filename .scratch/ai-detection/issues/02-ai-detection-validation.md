# Issue 02: AI Detection Validation Schemas
...
Create Zod validation schemas for all AI Detection inputs. These schemas will be used by both the service layer and API routes for consistent validation.

## Acceptance Criteria

- [ ] `lib/validation/ai-detection.ts` exists with all AI detection schemas
...
```typescript
// Detection types
export const aiDetectionTypeEnum = z.enum([
  'image',
  'video',
  'audio',
  'document',
...
export const aiDetectionCategoryEnum = z.enum([
  'low',
  'medium',
  'high',
]);
...
// Review decisions
export const aiReviewDecisionEnum = z.enum([
  'confirmed',
  'false_positive',
  'uncertain',
]);
...
// Queue status
export const aiQueueStatusEnum = z.enum([
  'pending',
  'processing',
  'completed',
  'failed',
...
// Queue priority
export const aiQueuePriorityEnum = z.enum([
  'low',
  'normal',
  'high',
]);
...
```typescript
// Helper to validate confidence score
export function validateConfidenceScore(score: number): number {
  return Math.min(100, Math.max(0, Math.round(score)));
}

// Helper to determine category from confidence score
...
// Helper to determine if should be flagged
export function shouldFlag(category: AIDetectionCategory, thresholds: { low: boolean; medium: boolean; high: boolean }): boolean {
...
- Use `z.number().min().max()` for numeric ranges
- Add custom error messages where helpful
- Export schema types for use in services
- For API keys, consider adding a validation that checks format (but don't store actual keys in validation)
- Consider adding a schema for detection configuration (thresholds, etc.)
